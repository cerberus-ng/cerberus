...
*Setup instructions for a Gitian build of Cerberus Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cerberus
...
- [Building Cerberus Core](#building-cerberus-core)
...
Clone the git repositories for Cerberus Core and Gitian.
...
in the Cerberus Core repository under 'Fetch and build inputs' to install sources which require
...
To build Cerberus Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Cerberus Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cerberus/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cerberuscoin/cerberus
...
For example:
```bash
URL=https://github.com/crowning-/cerberus.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cerberus=${COMMIT} --url cerberus=${URL} ../cerberus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cerberus=${COMMIT} --url cerberus=${URL} ../cerberus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cerberus=${COMMIT} --url cerberus=${URL} ../cerberus/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cerberus git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cerberus/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cerberus.git
SIGPATH=/some/root/path/cerberus-detached-sigs.git

./bin/gbuild --url cerberus=${BTCPATH},signature=${SIGPATH} ../cerberus/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cerberus-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cerberus-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cerberus-osx-build.assert
```
...
[cerberus/gitian.sigs](https://github.com/cerberuscoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
