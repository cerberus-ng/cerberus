...
#if defined(NDEBUG)
# error "Cerberus Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        return 410000 * COIN; // premine for the project fund
    }
	
	if (nPrevHeight < 300) {
		nSubsidyBase = 1;	//anti instamine
	}else{
		nSubsidyBase = 12;  //standard reward
	}
	
    CAmount nSubsidy = nSubsidyBase * COIN;

    // yearly decline of production by 10% per year, projected 27m coins max by year 2050.
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 20; //  60.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 20; //  65.0% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 20; //  70.0% 
	if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 40; //  72.5% 
	if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 40; //  75.0% 
...
void ThreadScriptCheck() {
    RenameThread("cerberus-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // CERBERUS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CBS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CBS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CERBERUS : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CBS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CBS): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Cerberus Related Inventory Messages
